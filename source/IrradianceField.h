...
G3D_DECLARE_ENUM_CLASS(LightingMode,
    DIRECT_INDIRECT, DIRECT_ONLY, INDIRECT_ONLY);
...
protected:
    ...
    // For probe m_debugProbeIndex
    shared_ptr<Texture>                 m_probeDebugTexture;

    /** For all probes */
    shared_ptr<Texture>                 m_screenDebugTexture;

    /** For all probes */
    shared_ptr<Texture>                 m_texelDebugTexture;

    /** For all probes */
    shared_ptr<Texture>                 m_overheadViewDebugTexture;

    /** Probe used for m_probeDebugTexture */
    ...
    */
    ...
    /** Projection from world space to m_overheadViewDebugTexture pixels. Applied to xz, since y is always dropped. */
    Matrix3 debugOverheadViewProjectToPixelMatrix() const;

    void setupAndBindDebugArgs(UniformTable& args, RenderDevice* rd) const;
    ...
    /** Create a complete (deep) copy of this irradiancefield. */
    shared_ptr<IrradianceField> createCopy();
    ...
    static void timingCallback(int verbosity, const char* tag, const char* message, void* userData) {
        debugPrintf(message);
        debugPrintf("\n");
    }
...
public:
    ...
    void loadNewScene(const String& sceneName, const shared_ptr<Scene>& scene, 
        Vector3int32 probeCountsOverride, float maxProbeDistance, int irradianceCubeResolutionOverride = -1, int depthCubeResolutionOverride = -1);
    ...
    void saveVisualization(RenderDevice* rd, const String& basePrefix = "probe_");

    void setShaderArgs(UniformTable& args, const String& prefix);
    
    int debugProbeIndex() const {
        return m_debugProbeIndex;
    }

    void setDebugProbeIndex(int d) {
        // Clamped because we have to fix the GUI slider's maximum range before we know what is legal
        m_debugProbeIndex = clamp(d, 0, probeCount() - 1);
    }
    ...
    static shared_ptr<IrradianceField> create(const String& sceneFilename, const shared_ptr<Scene>& scene,
        Vector3int32 probeCountsOverride = Vector3int32(-1, -1, -1), float maxProbeDistance = -1.0f, int irradianceCubeResolutionOverride = -1);

    shared_ptr<Texture> probeDebugTexture() const {
        return m_probeDebugTexture;
    }
